...
sudo virt-install --connect qemu:///system --name $name --os-variant centos7.0 --network network=default --memory 2048 --vcpus 1 --disk pool=default,size=10,format=qcow2 --graphics=spice --location http://mirror.linux.duke.edu/pub/centos/7.7.1908/isos/x86_64/ --autostart
